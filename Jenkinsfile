	...
	steps { 
     ...
     stage( 'Scanner sonar') {
		...
		withSonarQubeEnv('sonar_server'){
                sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=devops -Dsonar.host.url=http://172.17.0.3:9000 -Dsonar.java.binaries=target -Dsonar.coverage.exclusions=**/mvm**,**/src/test/**,**/model/** -Dsonar.junit.reportPath=target/surefire-reports  -Dsonar.tests=target/surefire-reports" 
                }
          }
      }
 }
