	...
	steps { 
     ...
     stage( 'Scanner sonar') {
		...
		withSonarQubeEnv('sonar_server'){
                sh "${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=devops -Dsonar.host.url=${sonarUrl} -Dsonar.java.binaries=target -Dsonar.coverage.exclusions=**/mvm**,**/src/test/**,**/model/** -Dsonar.junit.reportPath=target/surefire-reports" 
                }
          }
      }
      /*stage( 'Quality Gate' ){
